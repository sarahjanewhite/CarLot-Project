...
public class CarLot {
  ...
  public void saveToDisk() {
        try (ObjectOutputStream OOS = new ObjectOutputStream(new FileOutputStream("carlot.txt"))) {
            OOS.writeObject(inventory);
            System.out.println("Inventory saved to disk.");
        } catch (IOException IO) {
            System.err.println("Error saving inventory to disk: " + IO.getMessage());
        }
    }

    @SuppressWarnings("unchecked")
    public void loadFromDisk() {
        try (ObjectInputStream OIS = new ObjectInputStream(new FileInputStream("carlot.txt"))) {
            ...
            System.err.println("Error loading inventory from disk: " + CNFE.getMessage());
        }
    }
